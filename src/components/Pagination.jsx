    ...
    <div className='flex my-8 py-2 px-4 bg-gray-400 w-fit mx-auto rounded-full shadow-lg'>
      ...
      {pageNumbers.map((pageNumber) => (
        <button
          ...
          className={`mx-5 py-2 px-4 rounded-full hover:scale-125 transition-all ${pageNumber === page ? 'bg-blue-500 hover:scale-100 transition-all' : ''}`}
        >
          {pageNumber}
        </button>
      ))}
      <button onClick={nextPage} disabled={page === totalPages} className="text-4xl hover:scale-125 transition-all cursor-pointer"><MdNavigateNext /></button>
    </div>
  );
};
